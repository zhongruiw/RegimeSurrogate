...
function read_fine_cluster(file)
    hfile = h5open("/Users/ree/Documents/Research/RegimeSurrogate/code/ClusteringDynamical/data/" * file * "_fine_cluster.hdf5")
    X = read(hfile["X"])
    Xc = read(hfile["Xc"])
    close(hfile)
    return X, Xc
...
function fine_cluster(x; n_clusters = 2000, file = false)
    ...
    X,Xc = norm_ord(X,Xc)
    if file != false
        hfile = h5open("/Users/ree/Documents/Research/RegimeSurrogate/code/ClusteringDynamical/data/" * file * "_fine_cluster.hdf5", "w")
        hfile["X"] = X
        hfile["Xc"] = Xc
        close(hfile)
    end
