...
function read_coarse_cluster_tree(file)
    hfile = h5open("/Users/ree/Documents/Research/RegimeSurrogate/code/ClusteringDynamical/data/" * file * "_coarse_cluster_tree.hdf5")
    τ = read(hfile["τ"])
    interv_timescales = read(hfile["interv_τ"])
    # n_timescales = length(τ)
    n_timescales = read(hfile["n_timescales"])
    adj_array = []
    adj_mod_array = []
    edge_numbers_array = []
    node_labels_array = []
    X_LN_array = []
    for t in 1:interv_timescales:n_timescales
...
function coarse_cluster_tree(X, dt, n_timescales,interv_timescales; file=false,q_min=1e-16)
    ...
    X_LN_array = []
    if file != false
        filename = "/Users/ree/Documents/Research/RegimeSurrogate/code/ClusteringDynamical/data/" * file * "_coarse_cluster_tree.hdf5"
        hfile = h5open(filename, "w")
        hfile["τ"] = τ[1:interv_timescales:n_timescales]
        hfile["interv_τ"] = interv_timescales
        hfile["n_timescales"] = n_timescales
    end
    for t in 1:interv_timescales:n_timescales
    ...
    return X_LN_array, adj_array, adj_mod_array, node_labels_array, edge_numbers_array, τ[1:interv_timescales:n_timescales]
end
